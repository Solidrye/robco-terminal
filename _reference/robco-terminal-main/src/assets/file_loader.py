...
class FileLoader:
    def __init__(self):
        if getattr(sys, 'frozen', False):
            ...
            # Running from source: use this file's location so paths work when
            # the project lives in _reference/ and cwd is elsewhere
            _here = os.path.abspath(os.path.dirname(__file__))
            _project_root = os.path.dirname(os.path.dirname(_here))
            self.base_path = os.path.join(_project_root, 'src')
